...
func (x *KubeControllerManagerConfiguration) CodecEncodeSelf(e *codec1978.Encoder) {
	...
	if x == nil {
		...
		if false {
			...
			const yyr2 bool = false
			yyq2[44] = x.Kind != ""
			yyq2[45] = x.APIVersion != ""
			var yynn2 int
			if yyr2 || yy2arr2 {
				r.EncodeArrayStart(46)
			} else {
				yynn2 = 44
				for _, b := range yyq2 {
					if b {
			...
			if yyr2 || yy2arr2 {
				...
				if false {
					...
					r.EncodeInt(int64(x.MaxDeadNodeCount))
				}
			} else {
				z.EncSendContainerState(codecSelfer_containerMapKey1234)
				r.EncodeString(codecSelferC_UTF81234, string("maxDeadNodeCount"))
				z.EncSendContainerState(codecSelfer_containerMapValue1234)
				yym162 := z.EncBinary()
				_ = yym162
				if false {
	...
	for yyj3 := 0; ; yyj3++ {
		...
		case "volumeConfiguration":
			if r.TryDecodeAsNil() {
				...
				yyv58 := &x.VolumeConfiguration
				yyv58.CodecDecodeSelf(d)
			}
		case "maxDeadNodeCount":
			if r.TryDecodeAsNil() {
				x.MaxDeadNodeCount = 0
			} else {
				x.MaxDeadNodeCount = int(r.DecodeInt(codecSelferBitsize1234))
			}
		case "kind":
			if r.TryDecodeAsNil() {
				x.Kind = ""
