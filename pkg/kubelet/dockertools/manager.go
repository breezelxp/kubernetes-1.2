...
type Job struct {
	...
	Vip         string `json:"vip"`
	Vport       string `json:"vport"`
	Vport_start string `json:"vport_start"`
	Vport_end   string `json:"vport_end"`
}
...
func (dm *DockerManager) killPodWithSyncResult(pod *api.Pod, runningPod kubecontainer.Pod) (result kubecontainer.PodSyncResult) {
	...
	if networkContainer != nil {
		...
		if getDockerNetworkMode(ins) != namespaceModeHost && isCNINetworkPod(pod) {
			...
			if err := dm.networkPlugin.TearDownPod(runningPod.Namespace, runningPod.Name, kubecontainer.DockerID(networkContainer.ID.ID), pod); err != nil {
		...
		if !usesHostNetwork(pod) {
			...
			if isCNINetworkPod(pod) {
				...
				if ok && (value != "") {
					...
					if err == nil {
						var ret bool
						var vip, vport, vport_start, vport_end string
						ret, vip, vport, vport_start, vport_end = TgwApply(pod.Status.Network.Address, r)
						if ret {
							dm.runtimeHelper.UpdateRunContainerEnv(pod, vip, vport_start, vport_end)
						} else {
							glog.Warningf("TgwApply failed! ret:%s vip:%s vport:%s", ret, vip, vport)
						}
					} else {
						glog.V(3).Infof("***** get pod.Annotations Unmarshal failed! err:%s ******", err)
					}
				} else {
					glog.V(3).Infof("***** get pod.Annotations err:%s, value:%s ******", ok, value)
				}
			}
...
func TgwApply(Rsip string, tgw_info api.TGWReference) (bool, string, string, string, string) {
	...
	if err != nil {
		glog.V(3).Infof("TgwApply err:%+v", err)
		return false, "", "", "", ""
	}
	json.Unmarshal(body, &respData)
	if respData.Errcode > 1 {
		glog.V(3).Infof("TgwApply return err, code:%+v msg:%+v******", respData.Errcode, respData.Errmsg)
		return false, "", "", "", ""
	}
	jobid := respData.Data.JobID
	vip := respData.Data.Vip
	vport := respData.Data.Vport
	vport_start := respData.Data.Vport_start
	vport_end := respData.Data.Vport_end
	glog.V(3).Infof("TgwApply jobid:%+v ", jobid)
	// check job result
	glog.V(3).Infof(" Check tgwapply result...")
	retries := 360
	for i := 1; i <= retries; i++ {
		body, _, err := readBody(call("GET", fmt.Sprintf("/api_service/v1/job/%s", jobid), nil))
		if err != nil && i == retries {
			glog.V(4).Infof("Check tgwapply result request err:%+v", err)
			//fmt.Println(err)
			return false, "", "", "", ""
		} else if err != nil {
			glog.V(4).Infof("Check tgwapply result Retry %+v", i)
			time.Sleep(10 * time.Second)
			continue
		}
		json.Unmarshal(body, &respData)
		if respData.Errcode > 1 {
			glog.V(4).Infof("Check tgwapply result return err, err: %+v ", respData.Errmsg)
			return false, "", "", "", ""
	...
	return true, vip, vport, vport_start, vport_end
}

func call(method, path string, data interface{}) (io.ReadCloser, int, error) {
	var server string = "so.ied.com"
