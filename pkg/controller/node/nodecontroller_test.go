...
func TestMonitorNodeStatusEvictPods(t *testing.T) {
	...
	for _, item := range table {
		nodeController := NewNodeController(nil, item.fakeNodeHandler,
			evictionTimeout, util.NewFakeAlwaysRateLimiter(), util.NewFakeAlwaysRateLimiter(), util.NewFakeAlwaysRateLimiter(), testNodeMonitorGracePeriod,
...
func TestCloudProviderNoRateLimit(t *testing.T) {
	...
	nodeController := NewNodeController(nil, fnh, 10*time.Minute,
		util.NewFakeAlwaysRateLimiter(), util.NewFakeAlwaysRateLimiter(), util.NewFakeAlwaysRateLimiter(),
...
func TestMonitorNodeStatusUpdateStatus(t *testing.T) {
	...
	for i, item := range table {
		nodeController := NewNodeController(nil, item.fakeNodeHandler, 5*time.Minute, util.NewFakeAlwaysRateLimiter(), util.NewFakeAlwaysRateLimiter(),
			util.NewFakeAlwaysRateLimiter(), testNodeMonitorGracePeriod, testNodeStartupGracePeriod, testNodeMonitorPeriod, nil, nil, 0, false)
		nodeController.now = func() unversioned.Time { return fakeNow }
		if err := nodeController.monitorNodeStatus(); err != nil {
			t.Errorf("unexpected error: %v", err)
		}
...
func TestCheckPod(t *testing.T) {
	...
	nc := NewNodeController(nil, nil, 0, nil, nil, nil, 0, 0, 0, nil, nil, 0, false)
