...
type NodeController struct {
	...
	// replication controller framework and store
...
func NewNodeController(
	...
	evictionsLimiter util.RateLimiter,
	nodeMonitorGracePeriod time.Duration,
	nodeStartupGracePeriod time.Duration,
	nodeMonitorPeriod time.Duration,
	clusterCIDR *net.IPNet,
	...
	nc := &NodeController{
		...
		podEvictor:                NewRateLimitedTimedQueue(deletionEvictionLimiter, evictionsLimiter),
		terminationEvictor:        NewRateLimitedTimedQueue(terminationEvictionLimiter, evictionsLimiter),
	...
	nc.rcStore.Store, nc.rcController = framework.NewInformer(
		&cache.ListWatch{
			ListFunc: func(options api.ListOptions) (runtime.Object, error) {
				return nc.kubeClient.Core().ReplicationControllers(api.NamespaceAll).List(options)
			},
			WatchFunc: func(options api.ListOptions) (watch.Interface, error) {
				return nc.kubeClient.Core().ReplicationControllers(api.NamespaceAll).Watch(options)
			},
		},
...
func (nc *NodeController) Run(period time.Duration) {
	...
	go nc.rcController.Run(wait.NeverStop)
	...
	for i := range nodes.Items {
		...
		if currentReadyCondition != nil {
			if node.Spec.Unschedulable {
				// We do not treat the node when it unschedulable
				if nc.cancelPodEviction(node.Name) {
					glog.V(2).Infof("Node %s is unschedulable, cancelled pod eviction", node.Name)
				}
			} else {
...
func (nc *NodeController) deletePods(nodeName string) (bool, error) {
	...
	for _, pod := range pods.Items {
		...
		// if the pod is managed by rc, ignore it
		_, err = nc.rcStore.GetPodControllers(&pod)
		if err != nil {
			glog.V(4).Infof("No controllers found for pod %s, %v", pod.Name, err)
			continue
		}
